	...
	<div class="body-content clearfix">
	    ...
	    <?php if (have_posts()) : ?>
				...
				<tbody>
					...
					<tr>
						...
						<td class="jam-audio"><?php // if audio file is available, display audio link
							...
							if ($audio_file) { ?>
								<a href="http://tedm.net/library/jam/<?php echo $audio_file; ?>.mp3" title="Play <?php the_title(); ?> audio" rel="bookmark"><span class="dashicons dashicons-controls-volumeon"></span></a>
							<?php } ?></td>

						<td class="jam-video"><?php // if video file is available, display audio link
							$video_file = get_field('video');
							if ($video_file) { ?>
								<a href="http://tedm.net/library/jam/<?php echo $video_file; ?>.mov" title="Play <?php the_title(); ?> video" rel="bookmark"><span class="dashicons dashicons-video-alt3"></span></a>
							<?php } ?></td>
            ...
            <?php else : ?>
		        ...
		        <h3>No jams found. Try a different search?</h3>
		...
		<aside class="jamsidebar clearfix" role="secondary">
			...
			<img src="http://www.tedm.net/library/jam/musicians.gif" style="margin-bottom:15px;" />
